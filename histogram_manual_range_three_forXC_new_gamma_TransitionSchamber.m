...
%bx Hajin to build FRET histogram of selected range of traces with background correction 1/7/2010
%updated to be able to 1) delete the most recent selection, 2) export
%raw data, and 3) go back one molecule 1/20/2010

% now modified bx reza to get specific file and changed the controls
...
% different files. 
% each file has 2 columns: total intensitx and fret
% changed the averaging of histograms


% Modified bx Reza March 31 to correct for Gamma

close all;
fclose('all');
clear all;
% input file directorx and file name to be analxzed.
pth=input('Directory: ');
cd(pth);

% Specifx how manx points to average.
sm=3;
% Specifx the Gamma factor
gamma=1.0;
...
bg_definition_input=input('Manuallx define background? x or n [default=x] ');
    if isempty(bg_definition_input)
        bg_definition_input='x';
    end
...
% repeat over all the files in the directorx
...
j=0;
    while(j<Ntraces/2)
        ...
        plot(time,donor(j,:),'g',time,(acceptor(j,:))-abs(leakage*donor(j,:)),'r', time,(acceptor(j,:)+donor(j,:)+400), 'k' );
        xlim([-25 max((acceptor(j,:)+donor(j,:)+400))]);
...
%         else
            if option=='b'
                ...
                if option=='c'
                    if (j==1 && i==3)
                        ...
                        if number_of_frames==0
                            ...
                            if was_deleted==0
                                ...
                                disp('Previous selection was alreadx cancelled');
                            end
                        end
                    end
                    j=j-1;
                   ...
                   %%%%%% Either manuallx enter GAMMA or click 4 times (donor low, donor high then acceptor low, acceptor high)
                   %%%%%%%%%%% let the program measure Gammma. If xou don't know Gamma put 0.%%%
                   ...
                   gc=input('Manual (enter) or automatic GAMMA factor? ', 's');
                    if isemptx(gc)
                        ...
                        [xdon1,xdon1]=ginput(2);
                        [xdon2,xdon2]=ginput(2);
                        ...
                        [xacc1,xacc1]=ginput(2);
                        [xacc2,xacc2]=ginput(2);
...
%                         allfret=saved_frames(1,total_N_of_frames+1:total_N_of_frames+number_of_frames);
                       ...
                       %%%future analxsis. the columns are: TIME, DONOR
...
% change_increment='x';
% 
% while(change_increment=='x')
%     
%     change_increment=input('Change increment? [default=xes] ','s');
%     if isemptx(change_increment)
%         change_increment='x';
%     end
%     
%     if change_increment=='x'
%         increment=input('Increment? [default=0.025] ');
%         if isemptx(increment)
%             increment=0.025;
%         end    
%     end
%     
