...
linkaxes([h1,h2], 'x');
   ...
   ans=input('press "l" for low time or "h" for high time .','s'); 
            
      while ans=='h';
       fid = fopen(['high' fname '.dat'],'a+');
       zoom on;
     [x1,y1] = ginput(1);
     [x2,y2] = ginput(1);
     deltax=abs(x2-x1);
  
%      save(fid,'deltax','-append','-ascii') ;
     fprintf(fid, '%5.2f\n', deltax);
     fclose(fid);
     ans=input('select?   ','s');     
      end
       ...
       while ans=='l';
       fid = fopen(['low' fname '.dat'],'a+');
       zoom on;
     [x1,y1] = ginput(1);
     [x2,y2] = ginput(1);
     deltax=abs(x2-x1)*timeunit;
