...
manual=input('do you want to go through traces manually [y/n]?  ');
if isempty(manual)
        manual='y';
end
...
patchline(time(1:range),I_a_filter(1:range)+I_d_filter(1:range), 'linewidth',1.5,'edgecolor','k');
title(['File: ' fname '   Molecule:' int2str(nm-2)]);
ylim([0 20+max(I_a_filter(1:range)+I_d_filter(1:range))]);
...
%%%%%%%%%%%%%%% SAVE FILTERED TRACE %%%%%%%%%%%%
filtername=['filtered_' fname ];
filter_out=[I_d_filter I_a_filter];
rawname=['raw_' fname ];
raw_out=[ donor acceptor];
save([pathput '\' filtername '.dat'],'filter_out','-ascii') ;
save([pathput '\' rawname '.dat'],'raw_out','-ascii') ;
...
%%%%% TIME  raw data  smoothed fret  filtered fret   filtered donor/acceptor 
